...
The user wants to create a performance team of AI subagents for their project.
...
- FAIL → loop back to an earlier agent to investigate or fix

Example performance team flow:
  Baseline Recorder → Load Tester → Profiler → Regression Detector → DONE
  Load Tester:        PASS → Profiler,             FAIL → Baseline Recorder
  Regression Detector: PASS → DONE,               FAIL → Profiler

Keep it simple — 3 to 6 agents. Focus on establishing baselines, running load tests, profiling bottlenecks, and catching regressions.

## What to return

If you need clarification (e.g. performance targets, SLAs, existing tooling) before defining the plan, ask your questions in plain text now.
